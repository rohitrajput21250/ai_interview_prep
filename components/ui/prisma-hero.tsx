...
export const WordsPullUp = ({ text, className = "", showAsterisk = false, style }: WordsPullUpProps) => {
  ...
  return (
    <div ref={ref} className={`inline-flex flex-wrap items-center ${className}`} style={style}>
      {words.map((word, i) => {
          ...
          <div className="flex items-center gap-3 rounded-b-2xl bg-black px-4 py-2 sm:gap-6 md:gap-12 md:rounded-b-3xl md:px-8 lg:gap-14">
            ...
            {navItems.map((item) => (
              <a
                ...
                onMouseEnter={(e: React.MouseEvent<HTMLAnchorElement>) => (e.currentTarget.style.color = "#E1E0CC")}
                onMouseLeave={(e: React.MouseEvent<HTMLAnchorElement>) => (e.currentTarget.style.color = "rgba(225, 224, 204, 0.8)")}
              >
                {item}
              </a>
            ))}
            ...
            <div className="col-span-12 flex flex-col gap-5 pb-6 lg:col-span-4 lg:pb-10">
              ...
              <Link to="/dashboard">
                <motion.button
                  ...
                  className="group inline-flex items-center gap-2 self-start rounded-full bg-white text-black py-1 pl-5 pr-1 text-sm font-medium transition-all hover:gap-3 sm:text-base cursor-pointer"
                >
                  Begin Learning
